...
namespace SabzMarket.Application.UseCases.Orders.Checkout
{
    public class CheckoutOrderUseCase : ICheckoutOrderUseCase
    {
        ...
        public async Task<OperationResult> ExecuteAsync(long farmerId, CancellationToken token)
        {
            try
            {
                var cartItems = await _cartItemQueryService
                    .SelectByFarmerIdAsync(farmerId, token);

                var data = cartItems
                    .Where(x => x.Quantity > x.ProducNumber).ToList();
                ...
                if (!cartItems.Any())
                    throw new NotFoundException(Messages.CartEmpty);
                ...
                foreach (var item in cartItems)
                {
                    ...
                    {
                        var orderId = await _orderRepository
                            .FindOrderByFarmerAndSellerAsync(farmerId, item.SellerId, token);
