...
{
    public partial class FrmChat : FormStyle
    {
        public FrmChat()
        {
            InitializeComponent();
        ...
        public bool openFromSellerPage;
        ...
        private async void btnSend_Click(object sender, EventArgs e)
        {
            if (txtMessage.Text != "")
            {
                if (user.Id != 0)
                {
                    string receiverUserName = user.Username!;
        ...
        private async void frm_Chat_Load(object sender, EventArgs e)
        {
            ...
            if (openFromSellerPage)
            {
                ShowMessage(user);
            }
        }
        private async void ShowMessage(findUsersChattedOutputViewModel model)
        {
            pnlTitle.Visible = true;
            pnlMessage.Visible = true;
            tableLayoutPanel1.Visible = true;
            var result = await connection.InvokeAsync<bool>("IsOnlineUser", model!.Username);
            if (result)
            {
            ...
            var clientt = HttpClientHelper.Instance;
            var rout = string.Format(ApiRoutes.GetMessage, CurrentUser.UserId, model.Id);
            var messagee = await clientt.GetAsync<OperationResult<List<GetMessageOutputViewModel>>>(rout);
            ReadMessage(messagee.Data);
        }
